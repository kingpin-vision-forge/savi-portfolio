...
// SAVI brand colors - water blues and greens
const defaultColors = ['#0056d6', '#00c7fc', '#00C853'];
...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    class Advection extends ShaderPass {
      ...
      updatePass(args: { dt?: number; isBounce?: boolean; BFECC?: boolean }) {
    ...
    class ExternalForce extends ShaderPass {
      ...
      updatePass(props: { mouse_force?: number; cellScale?: THREE.Vector2; cursor_size?: number }) {
    ...
    class Viscous extends ShaderPass {
      constructor(simProps: Record<string, unknown>) {
        super({
          material: { vertexShader: face_vert, fragmentShader: viscous_frag, uniforms: { boundarySpace: { value: simProps.boundarySpace }, velocity: { value: (simProps.src as THREE.WebGLRenderTarget).texture }, velocity_new: { value: (simProps.dst_ as THREE.WebGLRenderTarget).texture }, v: { value: simProps.viscous }, px: { value: simProps.cellScale }, dt: { value: simProps.dt } } },
          output: simProps.dst, output0: simProps.dst_, output1: simProps.dst
        });
        this.init();
      }
      updatePass(args: { viscous?: number; iterations?: number; dt?: number }) {
    ...
    class Divergence extends ShaderPass {
      constructor(simProps: Record<string, unknown>) {
        super({
          material: { vertexShader: face_vert, fragmentShader: divergence_frag, uniforms: { boundarySpace: { value: simProps.boundarySpace }, velocity: { value: (simProps.src as THREE.WebGLRenderTarget).texture }, px: { value: simProps.cellScale }, dt: { value: simProps.dt } } },
          output: simProps.dst
        });
        this.init();
      }
      updatePass(args: { vel?: THREE.WebGLRenderTarget }) {
    ...
    class Poisson extends ShaderPass {
      constructor(simProps: Record<string, unknown>) {
        super({
          material: { vertexShader: face_vert, fragmentShader: poisson_frag, uniforms: { boundarySpace: { value: simProps.boundarySpace }, pressure: { value: (simProps.dst_ as THREE.WebGLRenderTarget).texture }, divergence: { value: (simProps.src as THREE.WebGLRenderTarget).texture }, px: { value: simProps.cellScale } } },
          output: simProps.dst, output0: simProps.dst_, output1: simProps.dst
        });
        this.init();
      }
      updatePass(args: { iterations?: number }) {
    ...
    class Pressure extends ShaderPass {
      constructor(simProps: Record<string, unknown>) {
        super({
          material: { vertexShader: face_vert, fragmentShader: pressure_frag, uniforms: { boundarySpace: { value: simProps.boundarySpace }, pressure: { value: (simProps.src_p as THREE.WebGLRenderTarget).texture }, velocity: { value: (simProps.src_v as THREE.WebGLRenderTarget).texture }, px: { value: simProps.cellScale }, dt: { value: simProps.dt } } },
          output: simProps.dst
        });
        this.init();
      }
      updatePass(args: { vel?: THREE.WebGLRenderTarget; pressure?: THREE.WebGLRenderTarget }) {
    ...
    class Simulation {
      ...
      update() {
        ...
        if (this.options.isViscous) vel = this.viscous.updatePass({ viscous: this.options.viscous, iterations: this.options.iterations_viscous, dt: this.options.dt });
        this.divergence.updatePass({ vel: vel! });
        const pressure = this.poisson.updatePass({ iterations: this.options.iterations_poisson });
        this.pressure.updatePass({ vel: vel!, pressure: pressure! });
      }
    }
    ...
    class WebGLManager implements LiquidEtherWebGL {
      ...
      initManager() { if (!Common.renderer) return; (this.props.$wrapper as HTMLElement).prepend(Common.renderer.domElement); this.output = new Output(); }
      ...
      dispose() { try { window.removeEventListener('resize', this._resize); if (this._onVisibility) document.removeEventListener('visibilitychange', this._onVisibility); Mouse.dispose(); if (Common.renderer) { const canvas = Common.renderer.domElement; if (canvas && canvas.parentNode) canvas.parentNode.removeChild(canvas); Common.renderer.dispose(); } } catch { /* noop */ } }
    }
  ...
  return <div ref={mountRef} className={`w-full h-full relative overflow-hidden ${className || ''}`} style={style} />;
}
